...
class Subscription(models.Model):
    ...
    date_of_order = models.DateField(auto_now_add=True, null=False, blank=False)
...
class SiteDonation(models.Model):
    ...
    date_of_donation = models.DateField(auto_now_add=True, null=False, blank=False)
    donation_amount_paid = models.DecimalField(max_digits=5, decimal_places=2, null=False, blank=False)
    
    def __str__(self):
        return "Donation details: {0} - {1} - £{2}".format(self.donated_user, self.date_of_donation, self.donation_amount_paid)